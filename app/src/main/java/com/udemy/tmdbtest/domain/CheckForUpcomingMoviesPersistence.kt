...
class CheckForUpcomingMoviesPersistence @Inject constructor(private val repository: MovieRepository) {
    ...
    suspend fun call(list: List<MovieModel>, networkAvailable: Boolean) {
        if (networkAvailable) {
            ...
            listToUpdate.forEach {
                val newMovie = it
                if(!it.type.contains(MovieType.UPCOMING.name)){
                    newMovie.type += ",${MovieType.UPCOMING.name}"
