...
class CheckForPlayingNowMoviesPersistence @Inject constructor(private val repository: MovieRepository) {
    ...
    suspend fun call(list: List<MovieModel>, networkAvailable: Boolean) {
        if (networkAvailable) {
            ...
            listToUpdate.forEach {
                val newMovie = it
                if(!it.type.contains(MovieType.PLAYING_NOW.name)){
                    newMovie.type += ",${MovieType.PLAYING_NOW.name}"
